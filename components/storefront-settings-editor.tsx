    ...
    <section className="storefront-editor-images">
      ...
      <div className="storefront-image-grid">
        ...
        <div className="product-color-field">
          <div><span>All product image backgrounds</span><small>One color is applied behind transparent product images everywhere in the store.</small></div>
          <div className="product-color-preview" style={{ backgroundColor: settings.product_background_color }} aria-label={`Product background preview ${settings.product_background_color}`}><i /><b>PRODUCT</b></div>
          <label className="product-color-picker"><span className="sr-only">Choose product background color</span><input type="color" value={settings.product_background_color} onChange={(event) => update("product_background_color", event.target.value)} /><strong>{settings.product_background_color.toUpperCase()}</strong></label>
          <label className="product-color-hex"><span>Hex color</span><input type="text" value={settings.product_background_color} maxLength={7} pattern="#[0-9a-fA-F]{6}" onChange={(event) => update("product_background_color", event.target.value)} /></label>
          <button type="button" onClick={() => update("product_background_color", "#e9ecf7")}>Reset color</button>
        </div>
      </div>
    </section>
